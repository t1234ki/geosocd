...
let darkMode = localStorage.getItem('electionbridge-theme') === 'dark';
...
function bindThemeToggle() { document.querySelectorAll<HTMLButtonElement>('.theme-toggle').forEach((button) => button.onclick = () => { darkMode = !darkMode; localStorage.setItem('electionbridge-theme', darkMode ? 'dark' : 'light'); applyTheme(); document.querySelectorAll<HTMLButtonElement>('.theme-toggle').forEach((item) => item.textContent = darkMode ? 'Light mode' : 'Dark mode'); }); }
applyTheme();
...
function authScreen(message = '') {
  ...
  document.querySelector<HTMLElement>('.auth-divider span')!.textContent = `Powered by ${brandName}`;
