...
function configurationScreen() { app.innerHTML = '<main class="config-page"><div class="config-card"><div class="brand"><span class="brand-mark">G</span> GEODues</div><p class="eyebrow">Configuration required</p><h1>Connect your workspace.</h1><p>Add <strong>VITE_SUPABASE_URL</strong> and <strong>VITE_SUPABASE_ANON_KEY</strong> to your local <strong>.env</strong> file, then restart Vite.</p></div></main>'; }
...
function onboardingScreen(message = '') {
  app.innerHTML = `<main class="config-page"><div class="config-card onboarding-card"><div class="brand"><span class="brand-mark">G</span> GEODues</div><p class="eyebrow">Association access</p><h1>Your account is pending access.</h1><p>This GEODues account is not linked to the association yet. Ask the association administrator to confirm your account and index number.</p>${message ? `<div class="form-error">${esc(message)}</div>` : ''}<button class="text-button" id="onboarding-logout">Sign out</button></div></main>`;
