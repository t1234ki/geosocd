...
const brandName = 'GEOSOC-UCC';
const productName = 'GEOSOC-UCC Dues';
const providerName = 'ElectionBridge';
let darkMode = localStorage.getItem('geosoc-ucc-theme') === 'dark';
...
function bindThemeToggle() { document.querySelectorAll<HTMLButtonElement>('.theme-toggle').forEach((button) => button.onclick = () => { darkMode = !darkMode; localStorage.setItem('geosoc-ucc-theme', darkMode ? 'dark' : 'light'); applyTheme(); document.querySelectorAll<HTMLButtonElement>('.theme-toggle').forEach((item) => item.textContent = darkMode ? 'Light mode' : 'Dark mode'); }); }
applyTheme();
...
function authScreen(message = '') {
  app.innerHTML = `<main class="auth-page"><section class="auth-showcase"><div class="brand auth-brand"><span class="brand-mark">G</span> GEOSOC-UCC</div><div class="auth-copy"><p class="eyebrow">The clear way to collect dues</p><h1>Every contribution.<br><em>Right on time.</em></h1><p>Secure member payments, receipts, and association visibility.</p><div class="auth-proof"><span class="proof-mark">+</span><div><b>Built for your association</b><small>Secure access, Paystack checkout, receipts, and admin visibility.</small></div></div></div><div class="auth-foot">GEOSOC-UCC <span>Secure contributions</span></div></section><section class="auth-panel"><div class="auth-panel-inner"><div class="auth-tabs"><button class="${authView === 'login' ? 'active' : ''}" data-auth-view="login">Log in</button><button class="${authView === 'signup' ? 'active' : ''}" data-auth-view="signup">Create account</button></div><p class="eyebrow">${authView === 'login' ? 'Welcome back' : 'Create your account'}</p><h2>${authView === 'login' ? 'Sign in to GEOSOC-UCC Dues' : 'Join GEOSOC-UCC Dues'}</h2><p class="auth-subtitle">${authView === 'login' ? 'Your account determines the dashboard you can access.' : 'Your administrator will assign your association access.'}</p>${message ? `<div class="form-error">${esc(message)}</div>` : ''}<form id="auth-form" class="auth-form">${authView === 'signup' ? '<label>Index number<input id="auth-index" type="text" placeholder="e.g. GEO/2026/001" required></label><label>Full name<input id="auth-name" type="text" placeholder="Your full name" required></label>' : ''}<label>Email address<input id="auth-email" type="email" placeholder="you@example.com" required></label><label>Password<input id="auth-password" type="password" placeholder="At least 8 characters" minlength="8" required></label>${authView === 'signup' ? '<label>Phone number<input id="auth-phone" type="tel" placeholder="+234 800 000 0000"></label>' : ''}<button class="primary-button auth-submit" type="submit">${authView === 'login' ? 'Continue to dashboard' : 'Create account'}</button></form>${authView === 'login' ? '<button class="text-button" id="forgot-password">Forgot password?</button>' : ''}<p class="auth-hint">${authView === 'login' ? 'Use the email assigned to your GEOSOC-UCC account.' : 'Your index number identifies your member account.'}</p><div class="auth-divider"><span>Powered by ElectionBridge</span></div><p class="auth-legal">By continuing, you agree to the GEOSOC-UCC terms and privacy policy.</p></div></section></main>`;
  document.querySelector<HTMLElement>('.auth-brand')!.innerHTML = `<span class="brand-mark">E</span> ${brandName}`;
  document.querySelector<HTMLElement>('.auth-foot')!.innerHTML = `${productName} <span>Powered by ${providerName}</span>`;
  ...
  document.querySelector<HTMLElement>('.auth-proof b')!.textContent = `Powered by ${providerName}`;
  ...
  document.querySelector<HTMLElement>('.auth-divider span')!.textContent = `Powered by ${providerName}`;
  document.querySelector<HTMLElement>('.auth-panel h2')!.textContent = authView === 'login' ? `Sign in to ${productName}` : `Join ${productName}`;
  document.querySelector<HTMLElement>('.auth-subtitle')!.textContent = authView === 'login' ? 'Your account determines the workspace you can access.' : 'Your administrator will assign your association access.';
  document.querySelector<HTMLButtonElement>('.auth-submit')!.textContent = authView === 'login' ? 'Continue to workspace' : 'Create account';
  document.querySelector<HTMLElement>('.auth-hint')!.textContent = authView === 'login' ? 'Use the email assigned to your association account.' : 'Your index number identifies your member account.';
  document.querySelector<HTMLElement>('.auth-legal')!.innerHTML = `By continuing, you agree to the ${brandName} terms and <a href="?privacy">privacy policy</a>.`;
  ...
  document.querySelector<HTMLElement>('.auth-showcase')!.insertAdjacentHTML('afterbegin', `<button class="theme-toggle auth-theme-toggle" type="button">${darkMode ? 'Light mode' : 'Dark mode'}</button>`);
  bindThemeToggle();
...
function showPaymentModal(title: string, content: string) {
  ...
  document.body.insertAdjacentHTML('beforeend', `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true"><button class="modal-close" id="modal-close" aria-label="Close">x</button><p class="eyebrow">GEOSOC-UCC</p><h2>${title}</h2>${content}</section></div>`);
...
function downloadReceipt(payment: Payment) {
  ...
  documentPdf.setFontSize(20); documentPdf.text('GEOSOC-UCC payment receipt', 20, 25);
    documentPdf.setFontSize(11); documentPdf.text('Institution: GEOSOC-UCC workspace', 20, 42); documentPdf.text(`Member: ${member?.full_name || 'Member'} (${member?.index_number || 'No index'})`, 20, 52); documentPdf.text(`Description: ${due?.title || 'GEOSOC-UCC payment'}`, 20, 62); documentPdf.text(`Amount: ${money(payment.amount)}`, 20, 72); documentPdf.text(`Date and time: ${new Date(payment.paid_at || payment.created_at).toLocaleString()}`, 20, 82); documentPdf.text(`Paystack reference: ${payment.paystack_reference}`, 20, 92); documentPdf.text(`Receipt number: ${receipt?.receipt_number || 'Pending'}`, 20, 102);
...
function paymentQrUrl(payment: Payment): string {
  ...
  const member = Array.isArray(payment.profiles) ? payment.profiles[0] : payment.profiles;
  const receipt = Array.isArray(payment.receipts) ? payment.receipts[0] : payment.receipts;
  const payload = JSON.stringify({ type: 'GEOSOC-UCC payment', reference: payment.paystack_reference, member: member?.index_number || profile?.index_number || 'Unknown', due: due?.title || 'GEOSOC-UCC payment', amount: money(payment.amount), paid_at: payment.paid_at || payment.created_at, receipt: receipt?.receipt_number || 'Pending' });
...
function showScannedPayment(rawValue: string) {
  try {
    ...
    if (!payment.reference || !['GEOSOC-UCC payment', 'GEOSOC-UCC ticket badge', 'GEODues payment', 'GEODues ticket badge'].includes(payment.type || '')) throw new Error('This is not a GEOSOC-UCC payment or ticket QR code.');
    const memberName = payment.member || 'Member';
    const memberInitial = memberName.trim().charAt(0).toUpperCase() || 'M';
    const detailPage = (payment.type === 'GEOSOC-UCC ticket badge' || payment.type === 'GEODues ticket badge')
      ? `<header class="topbar"><div><p class="eyebrow">Entry verification</p><h1>Ticket holder details</h1><p class="subtle">Scan result for event admission.</p></div></header><section class="verification-page"><div class="work-badge"><div class="work-badge-top"><div class="work-badge-avatar">${esc(memberInitial)}</div><div><p class="work-badge-kicker">GEOSOC-UCC verified badge</p><h2>${esc(memberName)}</h2><p class="verification-index">${esc(payment.index_number || 'No index number')}</p></div><div class="verification-status">${esc(payment.status || 'CONFIRMED')}</div></div><div class="verification-grid"><div><small>Event</small><strong>${esc(payment.event || 'Event ticket')}</strong></div><div><small>Ticket type</small><strong>${esc(payment.ticket_type || 'Ticket')}</strong></div><div><small>Amount paid</small><strong>${esc(payment.amount || 'Unknown')}</strong></div><div><small>Paid at</small><strong>${esc(payment.paid_at ? new Date(payment.paid_at).toLocaleString() : 'Unknown')}</strong></div><div><small>Status</small><strong>${esc(payment.status || 'CONFIRMED')}</strong></div><div><small>Reference</small><strong>${esc(payment.reference)}</strong></div></div></div><button class="ghost-button" id="verification-back">Back to dashboard</button></section>`
      : `<header class="topbar"><div><p class="eyebrow">Payment verification</p><h1>Payment details</h1><p class="subtle">Scan result.</p></div></header><section class="verification-page"><div class="work-badge"><div class="work-badge-top"><div class="work-badge-avatar">${esc(memberInitial)}</div><div><p class="work-badge-kicker">GEOSOC-UCC payment record</p><h2>${esc(memberName)}</h2></div><div class="verification-status">VERIFIED</div></div><div class="verification-grid"><div><small>Due</small><strong>${esc(payment.due || 'Payment')}</strong></div><div><small>Amount</small><strong>${esc(payment.amount || 'Unknown')}</strong></div><div><small>Paid at</small><strong>${esc(payment.paid_at ? new Date(payment.paid_at).toLocaleString() : 'Unknown')}</strong></div><div><small>Receipt</small><strong>${esc(payment.receipt || 'Pending')}</strong></div><div><small>Reference</small><strong>${esc(payment.reference)}</strong></div></div></div><button class="ghost-button" id="verification-back">Back to dashboard</button></section>`;
    document.querySelector<HTMLElement>('.modal-backdrop')?.remove();
    layout(detailPage);
